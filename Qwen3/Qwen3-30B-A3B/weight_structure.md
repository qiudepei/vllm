...
> **关于 RoPE 与位置扩展策略的完整说明**（含 Linear / NTK / YaRN / Llama-3 / ABF 等方案的对比），单独成文 → [rope_scaling.md](./rope_scaling.md)。
...
> **关于 LayerNorm / RMSNorm 公式对比、权重 shape 推导、`input_layernorm` 命名 ≠ 实现的反直觉**，单独成文 → [rmsnorm.md](./rmsnorm.md)。
...
下面两个主题如果展开会比较长，独立成专题文档，本文件只保留必要索引：

- **LayerNorm vs RMSNorm**：公式对比、RMSNorm 权重 shape 推导、`input_layernorm` 命名 ≠ 实现的反直觉  
  → [rmsnorm.md](./rmsnorm.md)

- **RoPE 与位置编码扩展**：Linear / NTK / YaRN / Llama-3 / ABF 等方案对比、Qwen3 为何 `rope_scaling=null`  
  → [rope_scaling.md](./rope_scaling.md)
